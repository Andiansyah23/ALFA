...
class SensitiveDataFilter(logging.Filter):
    def filter(self, record):
        record.msg = re.sub(r"'password': '.*?'", "'password': '[REDACTED]'", record.msg)
        return True
...
class AuthTester:
    def __init__(self, base_url):
        ...
        self.successful_usernames = set() # Retained as set for uniqueness, but can change to list if needed
        ...
        # Added from lower version for consistency
        self.otp_form = None # Store OTP form for reporting
        self.success_redirect_keywords = ['dashboard', 'home', 'profile', 'user', 'account', '/', 'settings', 'youraccount', 'licenses']
        
    async def handle_otp(self, crawler, account, login_form, auto_mode=None):
        """Handle OTP verification for an account dengan fitur abuse check dan stabilitas OTP"""
        print(f"[*] Processing OTP for {account['username']}")
        
        # Step 1: Get cookies from session after login
        ...
        # Dynamic detection of otp_form_url from redirect or crawling
        otp_keywords = ['otp', 'verification', 'two-factor', '2fa', 'twofactor', 'authenticator', 
                    'verification code', 'mfa', 'one-time', 'security code', 'pin', 'app_otp', 'totp']
        otp_form_url = None

        # Priority 1: Check if last_response.url is already OTP page
        ...
        # Priority 2: Extract links from last_response
        max_attempts = 5
        attempt = 0
        otp_form = None

        while attempt < max_attempts and otp_form is None:
            attempt += 1
            print(f"[*] Attempt {attempt}/{max_attempts} to find OTP form")
            ...
            if not otp_form_url:
                otp_form_url = input("[?] Enter OTP form URL manually (or 'quit' to cancel): ").strip()
                if otp_form_url.lower() == 'quit':
                    print("[-] OTP form selection cancelled by user.")
                    return False, False, None
                if not otp_form_url:
                    print("[-] No OTP form URL provided.")
                    continue

            # Step 2: Crawl OTP form
            ...
            if not otp_form:
                print(f"[-] OTP form not found at {otp_form_url}")
                otp_form_url = None  # Reset untuk mencoba lagi
                continue

            # Print crawled info
            # print(f"[+] Crawled Headers: {otp_form.get('headers', 'N/A')}")
            # print(f"[+] Crawled Cookies: {otp_form.get('cookies', 'N/A')}")
            print(f"[+] Header and Cookies found")
            ...
            confirm = input("[?] Is this the correct OTP form? (y/N/quit): ").strip().lower()
            if confirm == 'quit':
                print("[-] OTP form selection cancelled by user.")
                return False, False, None
            elif confirm != 'y':
                print("[-] OTP form rejected by user. Trying again...")
                otp_form = None
                otp_form_url = None  # Reset untuk mencari URL baru
                continue
            else:
                # Form diterima, keluar dari loop
                break

        # Jika setelah semua percobaan masih tidak mendapatkan form yang benar
        if otp_form is None:
            print("[-] Failed to find acceptable OTP form after multiple attempts.")
            return False, False, None
        ...
        abuse_choice = input("[?] Try checking OTP request abuse vulnerability? (y/N): ").strip().lower()
        if abuse_choice == 'y':
            otp_abuse_success = await self.otp_request_abuse(crawler, account, login_form)
            if otp_abuse_success:
                print("[+] Vulnerable: OTP request abuse successful")
            else:
                print("[-] Not vulnerable: OTP request abuse failed or blocked")
        ...
        if otp_auto:
            max_req = int(input("Max bruteforce try :"))
            otp_result = self.otp_bruteforce(otp_form, length, otp_type, max_req)
            if otp_result == "BLOCKED":
                print(f"[!] OTP brute detected for {account['username']}")
                return False, False, otp_abuse_success
            elif otp_result:
            ...
            while True:
                ...
                success = self.try_otp(otp_form, otp_code, base_url=self.base_url, otp_form_url=otp_form_url)
                if success:
        ...
        return success, otp_brute_success, otp_abuse_success
    ...
    def evaluate_login_success(self, response, pre_cookies=None):
        ...
        if (final_url != self.login_page_url and final_url != self.original_login_form.get('action', '')):
            ...
            if any(keyword in final_url for keyword in self.success_redirect_keywords):
                score += 1
    ...
    def evaluate_otp_success(self, response, original_otp_url):
        ...
        print("[*] Starting OTP evaluation score: 0")
        ...
        if response.status_code in [401, 403]:
            score -= 3
            print(f"[-] Failure: Unauthorized status code detected | Score now: {score}")
       
        # 1. Detect response redirect - Prioritaskan sukses jika ada redirect dan final URL bukan failure page
        failure_urls = ['login', 'otp', 'verify', 'two-factor', '2fa'] # Daftar URL indikasi failure/OTP
        if response.history:
            print("[+] Redirect detected")
            score += 2
            print(f"Score now: {score}")
            # PERBAIKAN: Hilangkan penalty untuk history yang mengandung failure (normal untuk redirect dari OTP page)
            # Penalty hanya jika FINAL URL adalah failure (e.g., loop back to OTP or login)
            if any(fail in final_url for fail in failure_urls):
                score -= 4
                print(f"[-] Failure: Final URL indicates redirect back to login/OTP page | Score now: {score}")
            else:
                # Prioritas tinggi: Jika redirect dan final URL bukan failure, tambah score tinggi karena strong indicator sukses
                print("[+] Redirect to non-failure URL - high success priority")
            ...
            # Penalty tambahan jika final URL sama dengan original OTP URL (stuck)
            ...
            if final_url == original_otp_url.lower() or any(fail in final_url for fail in failure_urls):
                ...
                print(f"[-] Stuck on OTP/failure URL without redirect | Score now: {score}")
            else:
                # Jika no redirect tapi URL berubah ke non-failure (e.g., AJAX success), bonus
                print("[+] No redirect but changed to non-OTP URL - assume success")
                score += 4
                print(f"Score now: {score}")
        ...
        for indicator in success_indicators:
            ...
            for tag_name in ['a', 'button', 'li', 'div', 'span', 'p', 'h1', 'h2', 'h3']:
                tag = soup.find(lambda tag: tag.name == tag_name and tag.text and indicator in tag.text.lower())
                if tag:
                    score += 1
                    print(f"[+] Found success indicator: '{indicator}' in <{tag_name}> tag | Score now: {score}")
                    found = True
                    break # Avoid multiple +1 for same indicator
            if not found and soup.find(attrs={'class': lambda c: c and any(cls in c for cls in ['success', 'info', 'welcome'])}):
                score += 1
                print(f"[+] Found success class for '{indicator}' | Score now: {score}")
        ...
        for elem in error_elements:
            ...
            for keyword_set in failure_keywords_sets:
                if all(kw in elem_text_lower for kw in keyword_set):
                    ...
                    print(f"[-] Strict failure with keywords: {', '.join(keyword_set)} in {elem.name} tag (text: {elem.text[:50]}...) | Score now: {score}")
            ...
            for indicator in general_failure_indicators:
                ...
                for p in p_tags:
                    ...
                    if indicator in text_lower and any(kw in text_lower for kw in otp_context_keywords):
                        score -= 2
                        print(f"[-] General failure: '{indicator}' in <p> tag with OTP context (text: {text_lower[:50]}...) | Score now: {score}")
                        found = True
                ...
                if not found:
                    for div in error_elements:
                        ...
                        if indicator in text_lower and any(kw in text_lower for kw in otp_context_keywords) and 'hidden' not in div.attrs:
                            score -= 2
                            print(f"[-] General failure: '{indicator}' in visible error div with OTP context (text: {text_lower[:50]}...) | Score now: {score}")
                            found = True
                # Add check in <span> or <h2> etc. with context in same element
                if not found:
                    found_tag = soup.find(lambda tag: tag.name in ['span', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'] and tag.text and indicator in tag.text.lower() and any(kw in tag.text.lower() for kw in otp_context_keywords))
                    if found_tag:
                        score -= 2
                        print(f"[-] General failure: '{indicator}' in {found_tag.name} tag with OTP context (text: {found_tag.text[:50]}...) | Score now: {score}")
       
        # Special handling for 'login': Check in <button> or <h1>-<h6>
        if soup.find(lambda tag: tag.name == 'button' and tag.text and 'login' in tag.text.lower()) or \
           any(soup.find(lambda tag: tag.name == f'h{i}' and tag.text and 'login' in tag.text.lower()) for i in range(1,7)):
            score -= 2
            print(f"[-] Failure indicator: 'login' in <button> or <h#> tag | Score now: {score}")
        ...
        for keyword in otp_keywords:
            ...
            if soup.find(lambda tag: tag.name == 'label' and tag.text and keyword in tag.text.lower()) or \
               soup.find('input', attrs={'placeholder': lambda p: p and keyword in p.lower()}):
                otp_detected = True
                break
        if otp_detected:
            score -= 1
            print(f"[-] OTP keywords present | Score now: {score}")
        else:
            score += 1
            print(f"[+] No OTP keywords | Score now: {score}")
       
        # Bonus jika cookie baru dan menandakan session sukses
        if len(response.cookies) > 0 and any('sess' in cookie.name.lower() or 'auth' in cookie.name.lower() for cookie in response.cookies):
            score += 2
            print(f"[+] New session/auth cookie detected | Score now: {score}")
       
        # Dynamic threshold: Lower jika redirect sukses
        threshold = 3 if any(keyword in final_url for keyword in self.success_redirect_keywords) else 4
    ...
    def reset_session(self):
        """Reset session for fresh attempts (adapted from lower version)"""
        self.session = requests.Session()
        self.last_response = None
    ...
    async def otp_request_abuse(self, crawler, account, login_form, max_attempts=10):
        """Perform OTP request abuse with warning and proper blocking detection"""
        print(f"[!] WARNING: Attempting OTP request abuse for {account['username']} may result in account or IP blocking.")
        print(f"[*] Attempting OTP request abuse for {account['username']}")
        success = False
        # Method 1: Detect resend link or form in OTP page
        otp_form_url = self.last_response.url if self.last_response else None
        if otp_form_url:
            result = await crawler.fetch(otp_form_url)
            if result:
                soup = BeautifulSoup(result['html'], 'html.parser')
                resend_keywords = ['resend', 'request', 'new', 'again', 'send', 'get', 'another', 'retry', 'regenerate', 'receive']
                # Check for resend link
                resend_links = soup.find_all('a', href=True, string=lambda text: text and any(kw in text.lower() for kw in resend_keywords))
                resend_url = None
                resend_method = 'get'
                resend_data = {}
                if resend_links:
                    resend_url = urljoin(otp_form_url, resend_links[0]['href'])
                    print(f"[+] Found resend OTP link: {resend_url}")
                else:
                    # Check for resend form/button
                    resend_forms = soup.find_all('form', string=lambda text: text and any(kw in text.lower() for kw in resend_keywords))
                    if resend_forms:
                        resend_form = resend_forms[0]
                        resend_url = urljoin(otp_form_url, resend_form.get('action', ''))
                        resend_method = resend_form.get('method', 'post').lower()
                        resend_data = {inp.get('name'): inp.get('value', '') for inp in resend_form.find_all('input') if inp.get('name')}
                        print(f"[+] Found resend OTP form: {resend_url} (method: {resend_method})")

                if resend_url:
                    for attempt in range(max_attempts):
                        if resend_method == 'post':
                            response = self.session.post(resend_url, data=resend_data, allow_redirects=True)
                        else:
                            response = self.session.get(resend_url, allow_redirects=True)
                        if response.status_code != 200 or self.detect_bruteforce(response):
                            print(f"[-] Abuse failed at attempt {attempt + 1} (Status: {response.status_code}, Blocked: {self.brute_force_detected})")
                            if self.brute_force_detected:
                                print("[!] Brute force or rate limit detected during OTP abuse. Stopping abuse attempt.")
                            return False
                        print(f"[*] Resend attempt {attempt + 1} successful")
                        time.sleep(random.uniform(1, 3))  # Increased delay to mimic user behavior
                    success = True
                    print("[+] OTP request abuse successful via resend (no limit detected)")
                    return success
        # Method 2: Repeated login attempts if resend not found
        print("[*] No resend found. Attempting abuse via repeated logins")
        successful_attempts = 0
        min_success_threshold = 5  # Increased for more reliable detection, but adjustable
        failed_login_count = 0
        max_failed_logins = 3
        for attempt in range(max_attempts):
            # Create fresh tester for each attempt to isolate state (e.g., response_sizes)
            temp_tester = AuthTester(self.base_url)
            temp_tester.login_page_url = self.login_page_url
            temp_tester.original_login_form = self.original_login_form
            print(f"[*] Abuse Attempt {attempt + 1}: Re-trying login with {account['username']}:[REDACTED]")
            fresh_login_form = temp_tester.refresh_login_form()
            if fresh_login_form:
                login_form = fresh_login_form  # Update to fresh form
                print("[+] Login form refreshed successfully")
            else:
                print("[-] Failed to refresh login form. Using existing form.")
            success_login, otp_detected = temp_tester.try_login(login_form, account['username'], account['password'])
            if temp_tester.brute_force_detected:
                print(f"[!] Abuse failed at attempt {attempt + 1}: Brute force or rate limit detected")
                self.brute_force_detected = True  # Propagate
                break
            if success_login and otp_detected:
                successful_attempts += 1
                print(f"[*] Login attempt {attempt + 1} successful, reached OTP page (Success count: {successful_attempts})")
            else:
                failed_login_count += 1
                if not success_login:
                    print(f"[-] Warning: Login unsuccessful at attempt {attempt + 1} (score low - check logs for details). Continuing...")
                if not otp_detected:
                    print(f"[-] Warning: OTP page not reached at attempt {attempt + 1}. Continuing...")
                if failed_login_count >= max_failed_logins:
                    print(f"[-] Too many failed logins ({failed_login_count}/{max_failed_logins}). Aborting abuse attempt.")
                    break
            time.sleep(random.uniform(2, 4))  # Longer delay
        if successful_attempts >= min_success_threshold:
            success = True
            print(f"[+] OTP request abuse successful via repeated logins (reached OTP {successful_attempts} times without limit detected)")
        else:
            print(f"[-] OTP request abuse failed (only reached OTP {successful_attempts} times)")
        return success